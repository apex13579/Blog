...
Danger
Touching Layer 1 cables without tracing your drops first will kill your management plane. Label both ends of every run before you disconnect anything.
...
Win
Linux Mint trades a slightly larger RAM footprint for immediate, out-of-the-box driver stability. It kept the deployment moving when Alpine stalled my entire night.
...
That fixed the storage layer, but the Nextcloud web UI immediately threw a domain trust error. I edited the config with nano, reached the login screen, then hit persistent database bugs blocking authentication. I finally pulled the plug on the whole container.

Warning
Nextcloud's permission implementation on TrueNAS SCALE has been a known, documented community issue for over two years. Don't waste days fighting it. Replace it with lightweight, decoupled apps instead.
...
01
Label everything
Use different colors for your patch cables. If you don't map both ends before you pull anything, Layer 1 will bite you every single time.

02
Ditch the monoliths
When an app fights your storage permissions for three hours straight, drop it. Single-purpose containers are dramatically easier to manage and debug.

03
Build CLI muscle memory
Googling basic commands inside a production VM is slow. Run Anki flashcards daily so the commands are there when you need them.
