# Professional Certifications & Academic Path

## 🏛️ Academic Degrees

* [ ] **A.S. I.T.**
* [ ] **B.S. I.T.**
* Utilize the B.S./M.S. pathway
* [ ] **M.S. I.T.**

## ✅ Completed Certifications

* [x] **FSU Customer Service**
* [x] **Google IT Support Professional**
* [x] **Google Project Management Professional**
* [x] **Google Data Analytics Professional**
* [x] **Google Cybersecurity Professional**

## 🚀 Planned Certifications & Training (Beginner to Expert)

### Beginner (Foundational)

* [ ] CompTIA Training: Professor Messer (YouTube)
* [ ] CompTIA Simulation/Prep: Boson ExSim-Max
* [ ] CompTIA A+
* [ ] CompTIA Network+
* [ ] CompTIA Security+
* [ ] Azure Training: John Savill's Masterclass / Microsoft Learn
* [ ] Azure Simulation/Prep: Boson ExSim-Max (AZ-900)
* [ ] Microsoft Certified: Azure Fundamentals (AZ-900)

### Intermediate (Professional)

* [ ] Azure Admin Training: Microsoft Learn / Pluralsight
* [ ] Azure Admin Simulation/Prep: Boson ExSim-Max (AZ-104)
* [ ] Microsoft Certified: Azure Administrator Associate (AZ-104)
* [ ] Linux Training: Shawn Powers (ITProTV) / CompTIA Linux+ materials
* [ ] Linux Simulation/Prep: Boson ExSim-Max (Linux+)
* [ ] CompTIA Linux+
* [ ] PenTest Training: TryHackMe / HTB Academy
* [ ] PenTest Simulation/Prep: Boson ExSim-Max (PenTest+)
* [ ] CompTIA PenTest+

### Expert (Advanced)

* [ ] HTB Training: Hack The Box Academy (CPTS Path)
* [ ] Hack The Box: Certified Penetration Testing Specialist (CPTS)
* [ ] HTB Training: Hack The Box Academy (CWEE Path)
* [ ] Hack The Box: Certified Web Exploitation Expert (CWEE)
