# Professional Certifications & Academic Path

## 🏛️ Academic Degrees
- [ ] **A.S. I.T.**
- [ ] **B.S. I.T.**
- [ ] **M.S. I.T.**

## ✅ Completed Certifications
- [x] **FSU Customer Service**
- [x] **Google IT Support Professional**
- [x] **Google Project Management Professional**
- [x] **Google Data Analytics Professional**
- [x] **Google Cybersecurity Professional**

## 🚀 Planned Certifications & Training (Beginner to Expert)

### Beginner (Foundational)
- [ ] Cisco Training: NetworkChuck Academy
- [ ] Cisco Simulation/Prep: Boson (NetSim & ExSim-Max)
- [ ] Cisco Certified Network Associate (CCNA)
- [ ] Azure Training: John Savill’s Masterclass / Microsoft Learn
- [ ] Azure Simulation/Prep: Boson ExSim-Max (AZ-900)
- [ ] Microsoft Certified: Azure Fundamentals (AZ-900)
- [ ] TCM Security: Practical Web Pentest Associate (PWPA)

### Intermediate (Professional)
- [ ] Azure Admin Training: Microsoft Learn / Pluralsight
- [ ] Azure Admin Simulation/Prep: Boson ExSim-Max (AZ-104)
- [ ] Microsoft Certified: Azure Administrator Associate (AZ-104)
- [ ] CCNP Training: Cisco U / INE / CBT Nuggets
- [ ] CCNP Simulation/Prep: Boson (NetSim & ExSim-Max)
- [ ] Cisco Certified Network Professional (CCNP Enterprise)
- [ ] LPI LPIC-1 Training: LPI Official Materials / Udemy (Imran Afzal)
- [ ] LPI LPIC-1 Simulation/Prep: Sybex / Whizlabs
- [ ] LPI: LPIC-1 Certified Linux Administrator (101-500 & 102-500)
- [ ] TCM Security: Practical Web Pentest Professional (PWPP)

### Expert (Advanced)
- [ ] CCIE Training: Cisco U / INE (Advanced Routing & Services)
- [ ] CCIE Simulation/Prep: Boson (ENCOR/CCIE Mastery)
- [ ] Cisco Certified Internetwork Expert (CCIE Enterprise)
- [ ] LPI LPIC-2 Training: LPI Official Materials / ITProTV (Don Pezet)
- [ ] LPI LPIC-2 Simulation/Prep: Udemy (Ken Adams)
- [ ] LPI: LPIC-2 Certified Linux Engineer (201-500 & 202-500)
- [ ] LPI: LPIC-3 Enterprise Professional (e.g., 303 Security Specialty)
- [ ] TCM Security: Practical Web Pentest Expert (PWPE)
